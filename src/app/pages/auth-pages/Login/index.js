...
import { loginAPI, registerAPI, registrationOTP, getOptAPI } from "app/services/auth-services";
const Login = () => {
    localStorage.clear()
    ...
    const headerSubmit = (values) => {
        ...
        if (values?.mobile && values?.securityCode) {
            if (!flag) {
                dispatch(loginAPI(values, response => {
        ...
        <>
            ...
            {/* <Formik
                            ...
                            onSubmit={(data, { setSubmitting }) => {
                                setSubmitting(true);
                                sendOtpFlag ? loginWithOtp(data) : sendOTP(data)
                                setSubmitting(false);
                            }}
                        >
                            {({ isSubmitting, values, errors, touched, setFieldValue }) => (
                                <Form style={{ textAlign: 'left' }} noValidate autoComplete='off'>
                                    <Div sx={{ mt: 1, mb: 3 }}>
                                        <InputMask mask="010-9999-9999" name="mobile" value={values.mobile} onChange={(e) => { setFieldValue('mobile', e.target.value); setMobile(e.target.value) }}>
                                            {() =>
                                                <TextField
                                                    required
                                                    onKeyDown={e => ["e", "E", "+", "-", "."].includes(e.key) && e.preventDefault()}
                                    ...
                                    {sendOtpFlag && <Div sx={{ mt: 1, mb: 1 }}>
                                        <InputMask mask="999999" name="securityCode" value={values.securityCode} onChange={(e) => { setFieldValue('securityCode', e.target.value) }}>
                                            {() =>
                                                <TextField
                                                    required
                                                    onKeyDown={e => ["e", "E", "+", "-", "."].includes(e.key) && e.preventDefault()}
                                    ...
                                    {sendOtpFlag && canResendOTP && <Typography variant={"body1"} style={{ cursor: 'pointer', textAlign: 'end' }} sx={{ mb: 2 }} onClick={() => sendOTP(values)}>
                                        resend OTP
                                    </Typography>}
                                    {sendOtpFlag && !canResendOTP && <Typography variant={"body1"} style={{ textAlign: 'end' }} sx={{ mb: 2 }} >
                                        Resend OTP in {countdown} seconds
                                    </Typography>}
                                    {sendOtpFlag && <LoadingButton
                                        fullWidth
                                        variant="contained"
                                        size="large"
                                        type='submit'
                                        // onClick={() => { loginWithOtp(values) }}
                                        loading={isSubmitting}

                                    >Verify and Login</LoadingButton>}
                                    {!sendOtpFlag && <LoadingButton
                                        fullWidth
                                        variant="contained"
                                        size="large"
                                    ...
                                    >Send One time password</LoadingButton>}
                                </Form>
                            )}
                        </Formik> */}
            ...
            {registerModel &&
                <Registration
                    open={registerModel}
                    handleClose={() => setRegisterModel(false)}
                    handleAgree={() => { setFlag(true); setRegisterModel(false); registrationAction() }}
                />
            }
        </>
    );
