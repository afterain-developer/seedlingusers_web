...
const JumboLayoutProvider = ({ children, activeLayout }) => {
    const [layoutOptions, setLayoutOptions] = React.useReducer(
        jumboLayoutReducer,
        {},
        init
    );
    if (activeLayout === "vertical-default") {
        layoutOptions.sidebar.width = 240
        layoutOptions.sidebar.minWidth = 80
    } else {
        layoutOptions.sidebar.width = 0
        layoutOptions.sidebar.minWidth = 0
    }
